...
# --- Global Constants for Memory Optimization ---
# Pre-allocate kernel to avoid recreation every frame
...
# Pre-calculate Rails List globally to prevent allocation churn
RAILS_LIST = [
    {
...
def draw_overlay(image, ghostball_center, white_line, extended_line, show_rails=False):
    ...
    # Draw rails if enabled (using global RAILS_LIST)
    if show_rails:
...
def main():
    # Create screenshots directory (absolute path)
    ...
    try:
        ...
        # Cleanup
        print("Stopping listener and cleaning up...")
        listener.stop()
