...
# Rails list is static, so we define it once to avoid per-frame allocation.
RAILS_LIST = [
    {
...
# Morphological kernel is static.
...
def draw_overlay(image, ghostball_center, white_line, extended_line, show_rails=False):
    """Draw the detection overlay on the image. Safe to modify image in-place."""
    # To optimize, we can draw directly on the image since the original frame
    # isn't used for anything else. However, to maintain the original logic's safety,
    # we'll use a reference, but generally in CV2 drawing is in-place.
    output = image # No copy!

    # Draw rails if enabled
    if show_rails:
        # Use global RAILS_LIST
    ...
    try:
        with mss.mss() as sct:
            while running[0]:
                ...
                # Convert BGRA to BGR - creates a new array
                ...
                # Draw overlay - modifies frame in-place to save memory
