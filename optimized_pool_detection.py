...
def get_rails_list():
    """Reconstruct rails list from flat endpoint dictionary."""
    rails = [
        {
...
def draw_overlay(image, ghostball_center, white_line, extended_line, rails_list, show_rails=False):
    """Draw the detection overlay on the image."""
    output = image.copy()

    # Draw rails if enabled
    if show_rails:
        # Use pre-calculated rails list passed as argument
...
def main():
    ...
    # Pre-calculate resources to prevent memory churn
    rails_list = get_rails_list()
    ...
    with mss.mss() as sct:
        try:
            while running[0]:
                ...
                # Detect ghostball - pass pre-allocated kernel
                ghostball_center = detect_ghostball(hsv_frame, kernel)
                ...
                # Draw overlay - pass pre-calculated rails
                output_frame = draw_overlay(frame, ghostball_center, white_line, extended_line, rails_list, show_rails[0])
