...
# Pre-calculate RAILS_LIST to avoid per-frame allocation
RAILS_LIST = [
    {
...
def draw_overlay(image, ghostball_center, white_line, extended_line, show_rails=False):
    """Draw the detection overlay on the image IN-PLACE."""
    # MODIFIED: No copy, modify in-place
    output = image

    # Draw rails if enabled (use global RAILS_LIST)
    if show_rails:
